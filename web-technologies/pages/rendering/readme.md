...
<a href="https://habr.com/ru/companies/odnoklassniki/articles/313978/">Слои, память, repaint, неявная композиция</a>  
___
...
&emsp;&emsp; 🔹 Layout  
&emsp;&emsp;&emsp;&emsp; 👆 Процесс <ins>[отрисовки геометрии 💬](## "with/height, position")</ins>    
&emsp;&emsp;&emsp;&emsp;&emsp;&emsp; 🔸 При изменении геометрии вызывает <ins>[Relayout || Reflow 💬](## "Пересчет расположения или размера")</ins> у всех элементов лежащих на одном слое 


<br>

&emsp;&emsp; 🔹 Paint  
&emsp;&emsp;&emsp;&emsp; 👆 Почти самый дешевый метод изменяющий только <ins>[внешний вид 💬](## "цвет, тень")</ins>   
...
&emsp;&emsp; 🔹 Юзаем только <ins>[css friendly 💬](## "transform, opacity")</ins> пропсы
...
&emsp;&emsp; 🔹 CSS Анимации крутяться на отдельном от `js` трейде, это позволяет продолжать ей работать, даже если <ins>[заблоикровали трейд](## "Например вызвали алерт")</ins>    
...
[![Edit custom-bind](https://codesandbox.io/static/img/play-codesandbox.svg)](https://codesandbox.io/s/optimize-loop-solution-c6943h?file=/src/index.js:0-1574)
...
Видео карта - Микро компьютер внутри компьютера, состаящая из нескольких состовляющих, куллера, GPU, и тд  
У пк мможет быть две видео карты, встроенная и дискретная  
Дискретная включаеться по мере повышения нагрузки   
Композиция - вынос элементов на GPU, и анимация силами gpu  
Repaint - изменения внешний вид элемента, но не его размеры  
...
Зависимые свойства так как высчитывают offsetParent и тд

---

#### Анимации могут работать на CPU, но CPU не так оптимизирован под это
* Нет слгаживания
* Качество анимации сильно зависит от загрузки CPU
* Анимации работают на CPU, при использовании свойст вызывающих repaint
