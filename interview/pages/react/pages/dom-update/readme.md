...
&emsp;&emsp; 👆 `requestAnimationFrame` помогает избежать проблем с пропуском кадров и дрожанием во время анимации, так как браузер старается обеспечить плавность анимации путем синхронизации с обновлением экрана.  
...
&emsp;&emsp; 👆 `requestIdleCallback` - это `API браузера`, которое позволяет выполнять функции во время простоя (idle) браузера, когда у него есть свободные ресурсы.
